...
`Tp` (Peak wave period) : Peak wave energy period, in seconds. A low `Tp` means a locally generated wind wave, while a high `Tp` means swell generated far away that has propagated to the location of interest	If you need to know whether the sea state is generated by local winds or from a distant storm.
`Tp` < 6 s : Mixed or wind wave environments
`Tp` > 8 s : Swell wave environments

Often this might be the only data available.

`Tp` is used to constrain locations by deployment suitability by deployment conditions. Deployment conditions affect the vessel's ability to deploy, and are also dependent on the type of vessel. Small tenders are assumed to be used for pilot deployment scenarios for 2025.
