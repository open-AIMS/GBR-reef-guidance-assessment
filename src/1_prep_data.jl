...
@showprogress dt = 10 "Prepping benthic/geomorphic/wave data..." for reg in REGIONS
    ...
    if !isfile(joinpath(RESULT_DIR, "$(reg)_waves.tif"))
        ...
        # reproj_region = region_features
        # reproj_region.geometry = AG.reproject(reproj_region.geometry, EPSG(4326), proj_str; order=:trad)

        target_waves_path = first(glob("*.nc", joinpath(DATA_DIR, "waves", reg)))
        target_waves = Rasters.Raster(target_waves_path, key=:Hs90, lazy=true, crs=EPSG(4326))
        target_waves = Rasters.trim(mask(target_waves; with=region_features[reg_idx, :]))
