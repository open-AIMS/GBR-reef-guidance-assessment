...
gbr_benthic_path = "$(ALLEN_ATLAS_DIR)/Benthic-Map/benthic.geojson"
# these are geojson files, not tif files
gbr_benthic = Raster(gbr_benthic_path, crs=EPSG(4326), lazy=true)


gbr_morphic_path = "$(ALLEN_ATLAS_DIR)/Geomorphic-Map/geomorphic.geojson"
# these are geojson files, not tif files
gbr_geomorphic = Raster(gbr_morphic_path, crs=EPSG(4326), lazy=true)

gbr_bathy_path = "$(ALLEN_ATLAS_DIR)/Bathymetry---composite-depth/bathymetry_0.tif"
gbr_bathy = Raster(src_bathy_path, mappedcrs=EPSG(4326), lazy=true)
